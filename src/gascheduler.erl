...
-spec execute(atom(), mfa(), task_callback()) -> ok.
execute(Name, MFA, Callback) ->
  gen_server:call(Name, {execute, MFA, Callback}).
...
execute_try(MFA, State = #state{nodes = Nodes,
  ...
  case get_free_node(Nodes, MaxWorkers, Running) of
    ...
    Node ->
      Args = [Scheduler, MFA, MaxRetries, RetryTimeout, Callback],
